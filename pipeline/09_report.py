...
<style>
  ...
  .caveat { background: #f1f8ff; padding: 14px; border-left: 4px solid #3498db; margin-top: 24px; }
...
<ul>
  <li>Marker-score-based subtyping is a proposal, not a classification. Cells with weak panel signal are flagged as <code>*_uncertain</code> or remain unrefined.</li>
  <li><code>IgG_rich_candidate</code> is the honest label. Mature plasma cell identity (CD138, MZB1, IgG protein) requires IF/IHC validation.</li>
  <li>This is one ROI. Reproducibility across multiple strips and patients is the next gate.</li>
  <li>Permutation tests use the spatial-bin null which preserves coarse tissue geography. This is conservative; the global label shuffle would call more pairs significant.</li>
</ul></div>

<p style="color:#888; margin-top:40px; font-size:12px;">
Generated by keloid_analysis_pipeline. See <code>provenance.json</code> in the
output folder for the exact config and git commit used.
</p>
</body></html>"""
